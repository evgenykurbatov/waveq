...
function propagate(wf::Wavefront{T}, wv::Real, pl2::Plane{T};
                   max_mem_mb::Real=2048, band_limited::Bool=true, method::Symbol=:simd) where T<:Real
    if method == :loop
        return _propagate_loop(wf, wv, pl2; band_limited=band_limited)
    elseif method == :simd
        return _propagate_simd(wf, wv, pl2; band_limited=band_limited)
    else
        throw(ArgumentError("Unknown propagate method: $method"))
    end
end

function _propagate_loop(wf::Wavefront{T}, wv::Real, pl2::Plane{T};
                         band_limited::Bool=true) where T<:Real
    nxi  = length(wf.xi)
    neta = length(wf.eta)
    nx2  = pl2.num_nodes[1]
    ny2  = pl2.num_nodes[2]
...
function _propagate_simd(wf::Wavefront{T}, wv::Real, pl2::Plane{T};
                         band_limited::Bool=true) where T<:Real
    nxi  = length(wf.xi)
    neta = length(wf.eta)
    nx2  = pl2.num_nodes[1]
    ...
    # Precompute longitudinal wavenumber and gradients
    kappa_mat = Matrix{Complex{T}}(undef, neta, nxi)
    grad_xi_mat = zeros(T, neta, nxi)
    grad_eta_mat = zeros(T, neta, nxi)
    ...
    total_pts = nx2 * ny2
    chunk_size = 256
    num_chunks = cld(total_pts, chunk_size)

    Threads.@threads for chunk_idx in 1:num_chunks
        start_idx = (chunk_idx - 1) * chunk_size + 1
        end_idx = min(chunk_idx * chunk_size, total_pts)
        S = end_idx - start_idx + 1

        # Local arrays for this chunk
        x_loc = zeros(T, S)
        y_loc = zeros(T, S)
        z_loc = zeros(T, S)
        u_chunk = zeros(Complex{T}, S)
        mask = zeros(Bool, S)
        ...
        for s in 1:S
            ...
            p0 = g .- pl1_orig
            x_loc[s] = dot(p0, pl1_ort_x)
            y_loc[s] = dot(p0, pl1_ort_y)
            z_loc[s] = dot(p0, pl1_ort_norm)
        end

        # Loop over spectrum (middle loop)
        for p in 1:nxi
            xi_p = xi_vec[p]
            for q in 1:neta
                U_pq = U_vec[q, p]
                ...
                # Precompute conditions if band_limited
                if band_limited
                    if imag(kappa) == 0.0 # propagating
                        gxi = grad_xi_mat[q, p]
                        geta = grad_eta_mat[q, p]
                        @inbounds @simd for s in 1:S
                            grad_phi_xi = 2π * x_loc[s] + gxi * z_loc[s]
                            grad_phi_eta = 2π * y_loc[s] + geta * z_loc[s]
                            mask[s] = (grad_phi_xi * wf.dxi)^2 + (grad_phi_eta * wf.deta)^2 <= π^2
                        end
                    else # evanescent
                        @inbounds @simd for s in 1:S
                            mask[s] = (2π * x_loc[s] * wf.dxi)^2 + (2π * y_loc[s] * wf.deta)^2 <= π^2
                        end
                    end
                else
                    fill!(mask, true)
                ...
                # Inner SIMD loop: accumulate phases
                @inbounds @simd for s in 1:S
                    if mask[s]
                        phase = 2π * xi_p * x_loc[s] + 2π * eta_q * y_loc[s] + kappa * z_loc[s]
                        u_chunk[s] += U_pq * cis(phase)
                    end
                end
            end
        end

        # Write chunk back to output buffer
        for s in 1:S
            idx = start_idx + s - 1
            i = div(idx - 1, ny2) + 1
            j = rem(idx - 1, ny2) + 1
            u_out[j, i] = u_chunk[s] * dxi_deta
        end
    end
